...
csv_file = File.read('C:\Users\ckimura\Downloads\PythonToCsv\CautionTags.csv')
csv_text = CSV.parse(csv_file, :headers => true)

CSV.open('output.csv', 'w', write_headers: true, headers: header) do |csv|
  CSV.foreach('C:\Users\ckimura\Downloads\PythonToCsv\CautionTags.csv', headers:true) do |row|
    csv << [row['Item Type'], blank, row['Product Name'], row['Product Type'], row['Product Code/SKU'], blank, blank, blank, row['Option Set Align'], blank, row['Price'], blank, blank, blank, blank, blank, blank, row['Product Weight'], blank, blank, blank, row['Allow Purchases?'], row['Product Visible?'], blank, blank, blank, blank, row['Category']]

...
header = ['Item Number', 'Item Name', 'Variant 1', 'Variant 2', 'Item Price', 'Image URL', 'Description']
...
CSV.open('output.csv', 'w', write_headers: true, headers: header) do |csv|
  CSV.foreach('input.csv', headers:true) do |row|
    ...
    if row['parent_id'].to_s != parent_id_last
      variant_all_combos << variant_1.to_a.product(variant_2.to_a).map{ |group| group.join(" ") }
